  ...
  while (1)
  {
    ...
    case RBF_FULL_INT:
      ...
      while (inp(port->com_LSR) & LSR_RBF)
      {
        ...
        if (port->xon_xoff_enabled == TRUE)
        {
          if (ch == XON)
          {
            if (port->xmit_interrupt_stopped == TRUE && port->output_blocked_by_xoff == TRUE && port->output_blocked_by_cts == FALSE && port->sbuff->count)
    ...
    case MODEM_STATUS_INT:
      ...
      if (port->rts_cts_enabled == TRUE)
      {
        ...
        {
          if (port->xmit_interrupt_stopped == TRUE && port->output_blocked_by_xoff == FALSE && port->output_blocked_by_cts == TRUE && port->sbuff->count)
            KickStartXmitInt(port);
...
BYTE SerialRead(s_com_port *port)
{
  ...
  if (rbuff->count) /* Test for presence of character(s) in buffer */
  {
    ...
    if (rbuff->count == port->rec_buff_minchars && port->input_blocked_by_flow_control == TRUE)
    {
      if (port->xon_xoff_enabled == TRUE)
...
int SerialWrite(s_com_port *port, BYTE ch)
{
  ...
  if (port->xmit_interrupt_stopped == TRUE && port->output_blocked_by_xoff == FALSE && port->output_blocked_by_cts == FALSE)
    KickStartXmitInt(port);
  _enable();
  return 0;
}
